...
class VAE_PBMC(nn.Module):
    def __init__(self):
        ...
        self.en1 = nn.Linear(self.input_size, self.hidden_dim)
        self.bn1 = nn.BatchNorm1d(num_features=self.hidden_dim)
        self.en_mu = nn.Linear(self.hidden_dim, self.latent_dim)
        self.en_std = nn.Linear(self.hidden_dim, self.latent_dim)
        self.bn2 = nn.BatchNorm1d(num_features=self.latent_dim)
        # self.bn3 = nn.BatchNorm1d(num_features=input_size)
        self.de1 = nn.Linear(self.latent_dim, self.hidden_dim)
        self.de2 = nn.Linear(self.hidden_dim, self.input_size)
        self.non_linear = self.non_linear_activation
    ...
    def encode(self, x):
        ...
        en_mu = self.en_mu(en1_relu)
        en_mu_bn = self.bn2(en_mu)
        en_mu_relu = self.non_linear(en_mu_bn)

        en_std = self.en_std(en1_relu)
        en_std_bn = self.bn2(en_std)
        en_std_relu = self.non_linear(en_std_bn)

        return en_mu_relu, en_std_relu
    ...
    def loss(self, recon_x, x, mu, log_var, L1_Lambda, KLD_Lambda):
        """gives the batch normalized Variational Error."""
        batch_size = x.size()[0]
        # reconstruction_loss = torch.nn.functional.mse_loss(recon_x, x.view(-1, input_size), reduction='mean')
        BCE_Loss = nn.BCELoss()
