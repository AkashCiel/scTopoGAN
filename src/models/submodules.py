...
class MLPAutoencoder_PBMC(AutoencoderModel):
    def __init__(self, arch=[50, 32 , 32, 8]):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Linear(arch[0], arch[1]),
            nn.BatchNorm1d(arch[1]),
            nn.ReLU(True),
            nn.Linear(arch[1], arch[2]),
            nn.BatchNorm1d(arch[2]),
            nn.ReLU(True),
...
class VAE_PBMC(nn.Module):
    def __init__(self, arch = [50, 32 , 32, 8]):

        super(VAE_PBMC, self).__init__()
        self.input_size = arch[0]
        self.hidden_dim = [arch[1], arch[2]]
        self.latent_dim = arch[3]
        self.non_linear = nn.ReLU()
        self.en1 = nn.Linear(self.input_size, self.hidden_dim[0])
        self.ebn1 = nn.BatchNorm1d(num_features=self.hidden_dim[0])
        self.en2 = nn.Linear(self.hidden_dim[0], self.hidden_dim[1])
        self.ebn2 = nn.BatchNorm1d(num_features=self.hidden_dim[1])
        self.en_mu = nn.Linear(self.hidden_dim[1], self.latent_dim)
        self.en_std = nn.Linear(self.hidden_dim[1], self.latent_dim)
        # self.bn3 = nn.BatchNorm1d(num_features=input_size)
        self.de1 = nn.Linear(self.latent_dim, self.hidden_dim[1])
        self.dbn1 = nn.BatchNorm1d(num_features=self.hidden_dim[1])
        self.de2 = nn.Linear(self.hidden_dim[1], self.hidden_dim[0])
        self.dbn2 = nn.BatchNorm1d(num_features=self.hidden_dim[0])
        self.output = nn.Linear(self.hidden_dim[0], self.input_size)
        #self.sigmoid = nn.Sigmoid()

        # Initialize all layers
        nn.init.xavier_uniform_(self.en1.weight)
        nn.init.xavier_uniform_(self.en2.weight)
        # nn.init.xavier_uniform_(self.en_mu.weight)
        # nn.init.xavier_uniform_(self.en_std.weight)
    ...
    def encode(self, x):
        ...
        en1 = self.en1(x)
        en1_bn = self.ebn1(en1)
        en1_relu = self.non_linear(en1_bn)
        
        en2 = self.en2(en1_relu)
        en2_bn = self.ebn2(en2)
        en2_relu = self.non_linear(en2_bn)
        
        mu = self.en_mu(en2_relu)

        log_var = self.en_std(en2_relu)
    ...
    def decode(self, z):
        ...
        de1 = self.de1(z)
        de1_bn = self.dbn1(de1)
        de1_relu = self.non_linear(de1_bn)
        
        de2 = self.de2(de1_relu)
        de2_bn = self.dbn2(de2)
        de2_relu = self.non_linear(de2_bn)

        #return self.sigmoid(self.output(de2_relu))
        return self.output(de2_relu)
    ...
    def loss(self, recon_x, x, mu, log_var, KLD_Lambda=1):
        """gives the batch normalized Variational Error."""
        batch_size = x.size()[0]
        reconstruction_loss = torch.nn.functional.mse_loss(recon_x, x.view(-1, self.input_size), reduction='mean')
        # BCE_Loss = nn.BCELoss()
        # reconstruction_loss = BCE_Loss(recon_x, x.view(-1, self.input_size))
        ...
        # Add L1 Loss
        # l1_loss = nn.L1Loss()
        # L1_Loss = l1_loss(recon_x, x.view(-1, self.input_size))

        return (reconstruction_loss + KLD_Lambda*KLD)
        # return (reconstruction_loss + KLD_Lambda * KLD + L1_Lambda * L1_Loss)
    ...
    def forward(self, x):
        ...
        return self.loss(x_reconst,x,mu,logvar)
...
class AE_PBMC(nn.Module):
    def __init__(self, arch = [50, 32 , 32, 8]):

        super(AE_PBMC, self).__init__()
        self.input_size = arch[0]
        self.hidden_dim = [arch[1], arch[2]]
        self.latent_dim = arch[3]
        self.non_linear = nn.ReLU()
        ...
        self.en2 = nn.Linear(self.hidden_dim[0], self.hidden_dim[1])
        self.ebn2 = nn.BatchNorm1d(num_features=self.hidden_dim[1])
        self.en_latent = nn.Linear(self.hidden_dim[1], self.latent_dim)
        self.de1 = nn.Linear(self.latent_dim, self.hidden_dim[1])
        self.dbn1 = nn.BatchNorm1d(num_features=self.hidden_dim[1])
        self.de2 = nn.Linear(self.hidden_dim[1], self.hidden_dim[0])
        self.dbn2 = nn.BatchNorm1d(num_features=self.hidden_dim[0])
