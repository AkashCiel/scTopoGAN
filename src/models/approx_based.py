...
class TopologicallyRegularizedVariationalAutoencoder(AutoencoderModel):
    """Topologically regularized autoencoder."""

    def __init__(self, lam=1., autoencoder_model='VAE_PBMC',
                 ae_kwargs=None, toposig_kwargs=None):
        """Topologically Regularized Autoencoder.
        ...
        toposig_kwargs = toposig_kwargs if toposig_kwargs else {}
        self.topo_sig = TopologicalSignatureDistance(**toposig_kwargs)
        print("will be using a Variational Autoencoder :)")
    ...
    def forward(self, x):
        ...
        """
        latent = self.autoencoder.get_z(x)
    ...
    def encode(self, x):
        return self.autoencoder.get_z(x)
