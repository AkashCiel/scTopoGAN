...
epochs = 1000
path_suffix = "Reduced_from_50_{}_{}".format(epochs, random_seed)
path_prefix = "Unsupervised_Manifold_Alignment/UnionCom"
data_dir = "Unsupervised_Manifold_Alignment/Data/PBMC Data"
...
if source_tech_name == "RNA":
  target_tech_name = "ATAC"
  # Define drive path to store the model
  model_code = "RNA_to_ATAC_TopoGAN"

else:
  target_tech_name = "RNA"
  # Define drive path to store the model
  model_code = "ATAC_to_RNA_TopoGAN"

source_tech = pd.read_csv("{}/{} Processed Reduced.csv".format(data_dir, source_tech_name), header=0, delimiter=',', index_col = 0)

target_tech = pd.read_csv("{}/{} Processed Reduced.csv".format(data_dir, target_tech_name), header=0, delimiter=',', index_col = 0)
...
# Write to file
tech_01_projected.to_csv(path_or_buf="{}/Integrated Data/UnionCom_Integrated_{}_{}.csv".format(path_prefix, path_suffix, source_tech_name), header=True, index=True)
tech_02_projected.to_csv(path_or_buf="{}/Integrated Data/UnionCom_Integrated_{}_{}.csv".format(path_prefix, path_suffix, target_tech_name), header=True, index=True)
...
def evaluate_first_k_neighbors(tech_01_projected, tech_01_projected_annotated,
    ...
    for i in range(len(all_indices)):
        ...
        sub_cell_type_source = tech_01_projected_annotated.iloc[[i]]["cell_type_00"].item()
        cell_type_source = tech_01_projected_annotated.iloc[[i]]["cell_type"].item()

        sub_cell_type_neighbors = get_majority_local_category(tech_02_projected_annotated, all_indices[i], "cell_type_00")
        cell_type_neighbors = get_majority_local_category(tech_02_projected_annotated, all_indices[i], "cell_type")
...
evaluation_results.to_csv(
    path_or_buf="{}/Evaluation Results/Evaluation Results_{}.csv".format(path_prefix, path_suffix),
